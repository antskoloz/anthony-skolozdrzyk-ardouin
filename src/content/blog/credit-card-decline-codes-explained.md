...
Whenever a card payment is declined, the issuer provides codes that your provider passes on to you. There are two kinds of network codes:
...
The meaning of a code differs by card network, so always read a code together with the card brand.

Providers also map network codes into their own labels. 
...
Instead of memorizing hundreds of codes, sort them by the response they need. 
...
No. The meaning differs by card network, so read the code together with the card brand.
